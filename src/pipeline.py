...
def process_all(cfg: dict) -> pd.DataFrame:
    ...
    if db_url.startswith("sqlite:/// "):
        ...
        db_url="sqlite:///"+str(root/db_url[10:])
            ...
            for page in page_data:
                ...
                if page["metodo"]=="ocr_pendente":
                    try: text=ocr_page(pdf,page["pagina"],cfg["ocr"]["dpi"],cfg["ocr"]["idioma"]); page["metodo"]="ocr"
                        ...
                        session.add(ErroProcessamento(
                            documento_id=doc.id,
                            pagina=page["pagina"],
                            etapa="ocr",
                            tipo=type(exc).__name__,mensagem=str(exc))
                        );
                        logging.exception("OCR falhou: %s p.%s",pdf.name,page["pagina"]); continue
                for raw in split_records(text):
                    ...
                    item=Atendimento(
                        ...
                        tempo_minutos=normalized.get("tempo_obj"),
                        status=row.get("status"),cep=row.get("cep"),
                        municipio=None,uf=None,
                        classificacao=classification,
                        motivos=row["motivos"],
                        texto_original=raw,
